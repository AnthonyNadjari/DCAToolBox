...
def validate() -> None:
    ...
    both = pd.concat(
        [pair["lev"].pct_change(), cl2.pct_change()], axis=1, keys=["synth", "real"]
    ).dropna()
    ...
    print(
        f"synthetic-2x vs real CL2: daily corr {corr:.3f}, "
...
def simulate(pair: pd.DataFrame, share_fn, seg: slice) -> dict:
    ...
    return {
        "final": round(float(v.iloc[-1])),
        "avg_share": round(ssum / max(ndep, 1), 3),
        "maxdd": round(dd, 3),
    }
...
def make_rules(pair: pd.DataFrame) -> dict:
    ...
    rules = {f"FIX_{int(s * 100)}": (lambda s: lambda i: s)(s) for s in (0.0, 0.25, 0.5, 0.75, 1.0)}
    # vol targeting: exposure E = min(2, target/vol); CL2 share = E - 1
    for tgt in (0.20, 0.25):
        rules[f"voltarget_{int(tgt * 100)}"] = (
            lambda t: (
                lambda i: (
                    np.clip(t / max(vol.iloc[i - 1], 1e-6) - 1.0, 0.0, 1.0)
                    if np.isfinite(vol.iloc[i - 1])
                    else 0.5
                )
            )
        )(tgt)
    # buy leverage in drawdowns (anti-cyclical: cheap beta after crashes)
    rules["dd_scaled"] = lambda i: (
        0.0
        if not np.isfinite(dd_ath.iloc[i - 1])
    ...
    rules["trend_gate"] = lambda i: (
        1.0 if (np.isfinite(sma200.iloc[i - 1]) and sma200.iloc[i - 1] > 0) else 0.0
    )
    # low-vol-regime leverage
    rules["lowvol_gate"] = lambda i: (
        1.0 if (np.isfinite(vol_pctl.iloc[i - 1]) and vol_pctl.iloc[i - 1] < 0.5) else 0.0
    )
    return rules
...
def main() -> None:
    ...
    for r in out:
        print(
            f"{r['name']:16s} {r['is']['final']:>10,} {r['is']['maxdd']:>6.0%} {r['is']['avg_share']:>6}   "
