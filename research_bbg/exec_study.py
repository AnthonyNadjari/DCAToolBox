...
def main() -> None:
    ...
    vwap = (df["trd_close"] * df["trd_volume"]).groupby(df["date"]).transform("sum") / df.groupby(
        "date"
    )["trd_volume"].transform("sum")
    ...
    print(f"{df['date'].nunique()} days, {len(df)} bars, {df.index.min()} -> {df.index.max()}\n")
    print(out.to_string())
